...
public class ThongKeNhapHang
    ...
    ArrayList<ThongKeNhapHangDTO> list;
    NhanVienBUS nVienBUS = new NhanVienBUS();
    JLabel lbsoluong, lbtongchiphi;
    PhieuNhapBUS pnBUS = new PhieuNhapBUS();

    public ThongKeNhapHang(ThongKeBUS thongkebus) {
        this.thongkebus = thongkebus;
        list = thongkebus.getAllNhapHang();
        initComponent();
        loadDataTable(list);
    }
    ...
    public void initComponent() {
        ...
        lbtongchiphi = new JLabel("Tông tiền chi: 0 VND");

        nhanviennhap = new InputForm("Tìm kiếm đơn nhập");
        nhanviennhap
                .getTxtForm()
        ...
        tblNH.setModel(tblModel);
        tblNH.setAutoCreateRowSorter(true);
        tblNH.setDefaultEditor(Object.class, null);
        scrollTblTonKho.setViewportView(tblNH);
        DefaultTableCellRenderer centerRenderer = new DefaultTableCellRenderer();
        centerRenderer.setHorizontalAlignment(JLabel.CENTER);
        tblNH.setDefaultRenderer(Object.class, centerRenderer);
        tblNH.setFocusable(false);
        tblNH.getColumnModel().getColumn(0).setPreferredWidth(10);
        tblNH.getColumnModel().getColumn(1).setPreferredWidth(50);
        tblNH.getColumnModel().getColumn(2).setPreferredWidth(200);
    ...
    private void tblNhapHangClicked(java.awt.event.MouseEvent evt) {
        // TODO add your handling code here:
        if (evt.getClickCount() == 2) {
            if (tblNH.getSelectedRow() == -1) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn đơn nhập");
            } else {
                ChiTietPhieuDialog ctp = new ChiTietPhieuDialog((JFrame) javax.swing.SwingUtilities.getWindowAncestor(this), "Thông tin đơn nhập", true, pnBUS.getSelect(getRow()));
            }
        }
    }

    public int getRow() {
        return tblNH.getSelectedRow();
    }
    ...
    public void loadDataTable(ArrayList<ThongKeNhapHangDTO> listphieunhap) {
        ...
        for (int i = 0; i < size; i++) {
            tblModel.addRow(
                    new Object[]{
                        i + 1,
                        "PN" + listphieunhap.get(i).getMaphieunhap(),
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == export) {
            try {
                JTableExporter.exportJTableToExcel(tblNH);
            } catch (IOException ex) {
                Logger
                        .getLogger(ThongKeNhapHang.class.getName())
