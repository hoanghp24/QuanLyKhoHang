...
public final class SanPham extends JPanel implements ActionListener {
    ...
    private void initComponent() {
        ...
        String[] header = new String[]{"Mã SP", "Tên sản phẩm", "Số lượng tồn", "Tác giả", "Thể loại", "Khu vực kho"};
    ...
    public void loadDataTalbe(ArrayList<DTO.SanPhamDTO> result) {
        ...
        for (DTO.SanPhamDTO sp : result) {
            tblModel.addRow(new Object[]{sp.getMasp(), sp.getTensp(), 
                sp.getSoluongton(), sp.getTacgia(), TheLoaiDAO.getInstance().selectById(sp.getTheloai()+"").getTentheloai(), 
                // NhaPhatHanhDAO.getInstance().selectById(sp.getNhaphathanh()+"").getTennph(),
