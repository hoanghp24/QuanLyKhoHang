...
public class ChiTietSanPhamDialog extends JDialog implements KeyListener, ItemListener {
    ...
    public void initComponent(String title) {
        ...
        String[] header = new String[]{"Mã vạch", "Mã phiếu nhập", "Mã phiếu xuất", "Tình trạng"};
    ...
    public void loadDataTable(ArrayList<ChiTietSanPhamDTO> result) {
        ...
        for (ChiTietSanPhamDTO ctsp : result) {
            tblModel.addRow(new Object[]{
                ctsp.getIsbn(), ctsp.getMaphieunhap(), ctsp.getMaphieuxuat() == 0 ? "Chưa xuất kho" : ctsp.getMaphieuxuat(), ctsp.getTinhtrang() == 1 ? "Tồn kho" : "Đã bán"
            });
        }
    ...
    public String[] getCauHinhPhienBan(int masp) {
        ...
        for (int i = 0; i < size; i++) {
            arr[i] = ngonNguBUS.getNgonNguById(ch.get(i).getNgonngu()) + " - " + loaiBiaBUS.getLoaiBiaById(ch.get(i).getLoaibia())
            + " - " + ch.get(i).getSotrang();
        }
        return arr;
    }
