...
public final class TheLoaiDialog extends JDialog implements MouseListener {
    ...
    ArrayList<TheLoaiDTO> list = tlBUS.getAll();
    QuanLyThuocTinhSP qltt;
    private final NhomQuyenBUS nhomquyenBus = new NhomQuyenBUS();

    public TheLoaiDialog(JFrame owner, QuanLyThuocTinhSP qlttsp, String title, boolean modal, int nhomquyen) {
    ...
    public void initComponent(QuanLyThuocTinhSP qltt) {
        ...
        headTite = new HeaderTitle("THỂ LOẠI SẢN PHẨM");
        ...
        ms = new InputForm("Tên thể loại");
        ms.setPreferredSize(new Dimension(250, 70));
        table = new JTable();
        table.setBackground(Color.WHITE);
        table.addMouseListener(this);
        tblModel = new DefaultTableModel();
        String[] header = new String[]{"Mã thể loại", "Tên thể loại"};
    ...
    public void loadDataTable(ArrayList<TheLoaiDTO> result) {
        tblModel.setRowCount(0);
        for (TheLoaiDTO th : result) {
            tblModel.addRow(new Object[]{
                th.getMatheloai(), th.getTentheloai()
            });
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == add) {
            if (Validation.isEmpty(ms.getText())) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập tên thể loại mới");
            } else {
                String tenngonngu = ms.getText();
                if (tlBUS.checkDup(tenngonngu)) {
                    int id = NgonNguDAO.getInstance().getAutoIncrement();
                    tlBUS.add(tenngonngu);
                    loadDataTable(list);
                    ms.setText("");
                } else {
            ...
            if (index != -1) {
                if (Validation.isEmpty(ms.getText())) {
                    ...
                    if (tlBUS.checkDup(tenthuonghieu)) {
                        tlBUS.update(new TheLoaiDTO(list.get(index).getMatheloai(), tenthuonghieu));
                        loadDataTable(list);
                        ms.setText("");
                    } else {
                        JOptionPane.showMessageDialog(this, "Thể loại đã tồn tại !");
            ...
            ms.setText(list.get(index).getTentheloai());
        }
    }
