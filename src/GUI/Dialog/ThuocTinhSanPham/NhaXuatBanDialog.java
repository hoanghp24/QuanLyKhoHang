...
public final class NhaXuatBanDialog extends JDialog implements MouseListener {
    ...
    ArrayList<NhaXuatBanDTO> list = nxbBUS.getAll();
    QuanLyThuocTinhSP qltt;
    private final NhomQuyenBUS nhomquyenBus = new NhomQuyenBUS();

    public NhaXuatBanDialog(JFrame owner, QuanLyThuocTinhSP qlttsp, String title, boolean modal, int nhomquyen) {
    ...
    public void initComponent(QuanLyThuocTinhSP qltt) {
        ...
        headTite = new HeaderTitle("NHÀ XUẤT BẢN SẢN PHẨM");
        ...
        ms = new InputForm("Tên nhà xuất bản");
    ...
    public void loadDataTable(ArrayList<NhaXuatBanDTO> result) {
        tblModel.setRowCount(0);
        for (NhaXuatBanDTO th : result) {
            tblModel.addRow(new Object[]{
                th.getManxb(), th.getTennxb()
            });
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == add) {
            if (Validation.isEmpty(ms.getText())) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập tên nhà xuất bản mới");
            } else {
                String tenngonngu = ms.getText();
                if (nxbBUS.checkDup(tenngonngu)) {
                    int id = NgonNguDAO.getInstance().getAutoIncrement();
                    nxbBUS.add(tenngonngu);
                    loadDataTable(list);
                    ms.setText("");
                } else {
                    JOptionPane.showMessageDialog(this, "Nhà xuất bản đã tồn tại !");
                }
            }
        } else if (e.getSource() == del) {
            int index = getRowSelected();
            if (index != -1) {
                nxbBUS.delete(list.get(index));
                loadDataTable(list);
                ms.setText("");
            }
            ...
            if (index != -1) {
                if (Validation.isEmpty(ms.getText())) {
                    ...
                    if (nxbBUS.checkDup(tenthuonghieu)) {
                        nxbBUS.update(new NhaXuatBanDTO(list.get(index).getManxb(), tenthuonghieu));
                        loadDataTable(list);
                        ms.setText("");
                    } else {
                        JOptionPane.showMessageDialog(this, "Nhà xuất bản đã tồn tại !");
            ...
            ms.setText(list.get(index).getTennxb());
        }
    }
