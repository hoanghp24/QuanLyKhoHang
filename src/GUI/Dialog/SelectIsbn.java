...
public class SelectIsbn extends JDialog{
    ...
    public SelectIsbn(JFrame owner, String title, boolean modal, TaoPhieuXuat taoPhieuXuat, ArrayList<ChiTietSanPhamDTO> ct){
        super(owner, title, modal);
        this.jTextArea = taoPhieuXuat.textAreaIsbn;
        this.ct = ct;
        init();
        setVisible(true);
    }
    
    public SelectIsbn(JFrame owner, String title, boolean modal, TaoPhieuKiemKe taoPhieuKiemKe, ArrayList<ChiTietSanPhamDTO> ct){
        super(owner, title, modal);
    ...
    public void init(){
        ...
        findImei.putClientProperty("JTextField.placeholderText", "Tìm kiếm mã vạch ...");
    ...
    public void loadImei(){
        ...
        for (ChiTietSanPhamDTO i : ct) {
            if (i.getIsbn().toLowerCase().contains(search)) {
                result.add(i);
            }
        }
        listMode.setSize(0);
        for (ChiTietSanPhamDTO chiTietSanPhamDTO : result) {
            CheckListItem check = new CheckListItem(chiTietSanPhamDTO.getIsbn());
            if(checkImeiArea(chiTietSanPhamDTO.getIsbn())){
