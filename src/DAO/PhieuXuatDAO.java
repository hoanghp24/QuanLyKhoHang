...
public class PhieuXuatDAO implements DAOinterface<PhieuXuatDTO> {
    ...
    public int insert(PhieuXuatDTO t) {
        ...
        try {
            ...
            String sql = "INSERT INTO `phieuxuat`(`maphieuxuat`, `tongtien`, `nguoitao`, `makh`, `trangthai`) VALUES (?,?,?,?,?)";
    ...
    public int update(PhieuXuatDTO t) {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "UPDATE `phieuxuat` SET `thoigian`=?,`makh`=?,`tongtien`=?,`trangthai`=? WHERE `maphieuxuat`=?";
    ...
    public int delete(String t) {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "DELETE FROM `phieuxuat` WHERE maphieunhap = ?";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
            pst.setString(1, t);
            result = pst.executeUpdate();
            JDBCUtil.closeConnection(con);
    ...
    public int getAutoIncrement() {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'quanlykhohang' AND TABLE_NAME   = 'phieuxuat'";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
